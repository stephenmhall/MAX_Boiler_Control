...
class MainWindow():
    ...
    def switchHeat(self):
        ...
        if boilerEnabled:
            try:
                responce = requests.get(veraControl.format(Vera_Address, Vera_Device, str(boilerState)), timeout=5)
            ...
            boilerState = 0
            try:
                responce = requests.get(veraControl.format(Vera_Address, Vera_Device, boilerState), timeout=5)
                DB.updateVeraState(1)
